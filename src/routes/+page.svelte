...
<div id="wrapper">
	...
	{#each contentItems as item}
		<Content title={item.title} text={item.text} imageURL={item.imageURL} alignment={contentItems.indexOf(item) % 2 ? "left" : "right"} />
	{/each}
</div>
